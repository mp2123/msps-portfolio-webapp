...
export function SpiralSignal({ title, subtitle, labels }: SpiralSignalProps) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-[1.8rem] border border-cyan-400/15 bg-gradient-to-br from-cyan-500/10 via-slate-950/90 to-black/95 p-5 shadow-[0_0_40px_rgba(34,211,238,0.08)]"
      onPointerMove={(event) => {
      ...
      <div className="grid gap-5 md:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="relative z-10 flex flex-col justify-center gap-4">
          <div className="space-y-3">
            ...
            <p className="text-sm leading-relaxed text-zinc-200/90 md:text-[0.95rem]">{subtitle}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {labels.map((label) => (
              <span
                key={label}
                className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.2em] text-zinc-100 shadow-[0_0_18px_rgba(34,211,238,0.05)]"
              >
                {label}
              </span>
            ))}
